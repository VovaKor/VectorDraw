...
public class SvgBasicElementHandler {
    ...
    void handleElement(SVGOMGElement element) {
        ...
        IGroup igroupResult = ShapeFactory.INST.createGroup();
//        igroupResult.
        result.setId(element.getId());
        ...
        loader.parentIGroup.getShapes().add(igroupResult);
        loader.parentIGroup = igroupResult;
...
//        loader.parentNode = result;
    ...
    void handleElement(SVGOMRectElement element) {
        ...
        //Creates VectorDraw rectangle model
        IRectangle rectangle = ShapeFactory.INST.createRectangle(ShapeFactory.INST.createPoint(xpos, ypos), width, height);
        rectangle.setLineArc(((cornerHeight*cornerHeight)+(cornerWidth*cornerWidth))/(2 * cornerHeight));
        
        Affine transformation = styleTools.getTransform(element);
        if (transformation != null) {
            result.getTransforms().add(transformation);
            //TODO: add other transformations 
            rectangle.translate(transformation.getTx(),transformation.getTy());
        }
                
        styleTools.applyStyle(result, element);
        styleTools.applyStyle(rectangle, element);

        loader.parentIGroup.getShapes().add(rectangle);
//        loader.parentNode.getChildren().add(result);
    }

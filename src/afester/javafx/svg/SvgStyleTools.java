...
public class SvgStyleTools {
    ...
    /**
     * Applies the styling of an svg element to a VectorDraw Shape model.
     *
     * @param shape   The VectorDraw Shape model to style.
     * @param element The SVG DOM element which defines the styling.
     */
    void applyStyle(IShape shape, SVGStylableElement element) {
        // fill
        IColor color = getFillingColor(element);
        if (color != null) {
            shape.setFillingCol(color);
            shape.setFilled(true);
        }
                
        // stroke
        color = getLineColor(element);
        if (color != null) {
            shape.setLineColour(color);
        }
        ...
        if (swidth != null) {
            ...
            shape.setThickness(strokeWidth);
        }
        ...
        if (strokeDashArray != null
            && strokeDashArray.getCssValueType() == CSSValue.CSS_VALUE_LIST) {
                ...
                double dashBlackLength = strokeDashArray.getValue().item(0).getFloatValue();
                double dashWhiteLength = strokeDashArray.getValue().item(1).getFloatValue();
                shape.setLineStyle(ELineStyle.DASHED);
                shape.setDashSepBlack(dashBlackLength);
                shape.setDashSepWhite(dashWhiteLength);
...
*/
    ...
    private IColor getFillingColor(SVGStylableElement element) {
        IColor color = null;
...
//            color = paints.get(uri);
        ...
        if (val.getPaintType() == SVGPaint.SVG_PAINTTYPE_RGBCOLOR) {
            ...
            CSSOMComputedStyle.ComputedCSSValue  opacity = 
                    (ComputedCSSValue) style.getPropertyCSSValue("fill-opacity");
            float alpha = opacity.getFloatValue(CSSPrimitiveValue.CSS_NUMBER);
            color = ShapeFactory.INST.createColor(red, green, blue, alpha);
        }

        return color;
    }

    private IColor getLineColor(SVGStylableElement element) {
        IColor result = null;
...
//            result = paints.get(uri);
        ...
        if (val.getPaintType() == SVGPaint.SVG_PAINTTYPE_RGBCOLOR) {
            ...
            result = ShapeFactory.INST.createColor(red, green, blue, alpha);
        }
